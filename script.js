...
function getSavedLanguage() {
  try {
    return localStorage.getItem("filefix-language") === "en" ? "en" : "ar";
  } catch (error) {
    return "ar";
  }
}
...
function applyLanguage(language) {
  ...
  try {
    localStorage.setItem("filefix-language", lang);
  } catch (error) {
    console.warn(error);
  }
}
...
function loadDarkMode() {
  try {
    if (
      localStorage.getItem("filefix-dark") === "1"
    ) {
...
/* FileFix: English is the default language. Arabic remains available from the language switcher. */
document.addEventListener("DOMContentLoaded", function () {
  try {
    if (!localStorage.getItem("filefix-language")) {
      localStorage.setItem("filefix-language", "en");
      if (typeof window.applyFileFixLanguage === "function") {
        window.applyFileFixLanguage("en");
      }
    }
  } catch (e) {
    console.warn("Language preference could not be saved.", e);
  }
});


/* Default language: English.
   Arabic is available only after the visitor explicitly switches to AR. */
document.addEventListener("DOMContentLoaded", function () {
  try {
    const key = "filefix-language";
    ...
    console.warn("Could not initialize language preference.", error);
  }
});
